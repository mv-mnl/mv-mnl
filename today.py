...
def get_github_avatar_ascii(username, width=39, height=25):
    try:
        ...
        img = Image.open(BytesIO(req_img.content)).convert("L")
...
BIRTHDAY = datetime.datetime(2005, 5, 15)
...
def svg_overwrite(
    ...
    template_file = "template_dark.svg" if "dark" in filename else "template_light.svg"
    tree = parse(template_file)
    root = tree.getroot()

    # Apply personalization
    replace_text_in_line(root, "{{ NAME_HEADER }}", USER_CONFIG["NAME_HEADER"])
    replace_text_in_line(root, "{{ OS }}", USER_CONFIG["OS"])
    replace_text_in_line(root, "{{ HOST }}", USER_CONFIG["HOST"])
    replace_text_in_line(root, "{{ KERNEL }}", USER_CONFIG["KERNEL"])
    replace_text_in_line(root, "{{ IDE }}", USER_CONFIG["IDE"])
    replace_text_in_line(root, "{{ LANG_PROG }}", USER_CONFIG["LANG_PROG"])
    replace_text_in_line(root, "{{ LANG_COMP }}", USER_CONFIG["LANG_COMP"])
    replace_text_in_line(root, "{{ LANG_REAL }}", USER_CONFIG["LANG_REAL"])
    replace_text_in_line(root, "{{ HOBBY_SW }}", USER_CONFIG["HOBBY_SW"])
    replace_text_in_line(root, "{{ HOBBY_HW }}", USER_CONFIG["HOBBY_HW"])
    replace_text_in_line(root, "{{ HOBBY_OTH }}", USER_CONFIG["HOBBY_OTH"])
    replace_text_in_line(root, "{{ EMAIL_PERS }}", USER_CONFIG["EMAIL_PERS"])
    replace_text_in_line(root, "{{ EMAIL_WORK }}", USER_CONFIG["EMAIL_WORK"])
    replace_text_in_line(root, "{{ INSTA }}", USER_CONFIG["INSTA"])
    replace_text_in_line(root, "{{ DISCORD }}", USER_CONFIG["DISCORD"])

    if ascii_lines:
        # SVG tags in lxml have no namespace wrapper when searching like ElementTree does
        # However, finding tags requires standard methods. Let's iterate:
        for text in root.findall(".//{http://www.w3.org/2000/svg}text"):
            if text.get("class") == "ascii":
                ...
                for line in ascii_lines:
                    # lxml needs the tag namespace explicit for new tags if the tree enforces it
                    ns = "http://www.w3.org/2000/svg"
                    tspan = Element(f"{{{ns}}}tspan", x="15", y=str(start_y))
                    tspan.text = line
                    text.append(tspan)
                    start_y += 20
